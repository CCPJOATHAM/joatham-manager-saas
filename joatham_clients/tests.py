...
class ClientPermissionTests(TestCase):
    ...
    def test_gestionnaire_cannot_delete_other_entreprise_client(self):
        self.client.force_login(self.gestionnaire_a)
        response = self.client.get(reverse("delete_client", args=[self.client_b.id]))
        self.assertEqual(response.status_code, 404)
        self.assertTrue(Client.objects.filter(id=self.client_b.id).exists())

    def test_gestionnaire_can_delete_own_client(self):
        self.client.force_login(self.gestionnaire_a)
        response = self.client.get(reverse("delete_client", args=[self.client_a.id]))

        self.assertRedirects(response, reverse("client_list"))
        self.assertFalse(Client.objects.filter(id=self.client_a.id).exists())

    def test_client_edit_requires_clients_module_access(self):
        self.plan.modules_inclus = ["dashboard"]
        self.plan.save(update_fields=["modules_inclus"])
        self.client.force_login(self.gestionnaire_a)

        response = self.client.get(reverse("edit_client", args=[self.client_a.id]))

        self.assertEqual(response.status_code, 302)
        self.assertIn("module=clients", response["Location"])

    def test_client_delete_requires_clients_module_access(self):
        self.plan.modules_inclus = ["dashboard"]
        self.plan.save(update_fields=["modules_inclus"])
        self.client.force_login(self.gestionnaire_a)
    ...
    def test_client_list_displays_empty_state(self):
        ...
        response = self.client.get(reverse("client_list"))
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "Aucun client enregistré pour le moment")
        self.assertContains(response, "Ajouter un client")
